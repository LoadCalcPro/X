...
function qty(id){
  const el=document.getElementById(id);
  const n=Math.floor(Number(el?el.value:0)||0);
  return n>0?n:0;
}
...
function generalTotal(){
  return Math.max(Number(value('q5'))||0,0)*3+
    Math.max(qty('q6'),0)*1500+
    Math.max(qty('q7'),0)*1500;
}

function applianceTotals(){
...
function loadRow(label,quantity,service,generator,className){
  if(Number(service)<=0&&Number(generator)<=0)return '';
  return '<tr class="'+(className||'normal-load-row')+'">'+
    '<td>'+esc(label)+'</td>'+
    '<td class="quantity">'+(quantity?esc(quantity):'')+'</td>'+
    '<td class="number">'+num(service)+'</td>'+
    '<td class="number">'+num(generator)+'</td>'+
    '</tr>';
}

function totalRow(label,service,generator,className){
  return '<tr class="'+(className||'subtotal-row')+'">'+
    '<td><strong>'+esc(label)+'</strong></td><td></td>'+
    '<td class="number"><strong>'+num(service)+'</strong></td>'+
    '<td class="number"><strong>'+num(generator)+'</strong></td></tr>';
}

function sectionRow(label){
  return '<tr class="print-section-row"><td colspan="4">'+esc(label)+'</td></tr>';
}

function applianceLabel(item){
  if(typeof applianceDescription==='function')return applianceDescription(item.row,item.label);
  return item.label||'';
}

function hvacLabel(row){
  if(typeof window.getHVACRowLabel==='function')return window.getHVACRowLabel(row);
...
function installStyle(){
  ...
  style.id='generatorPrintSingleSourceStyle';
  style.textContent=`
@media print{
...
window.updatePrintRows=function(data){
  ...
  const appliances=applianceTotals();
  const combinedService=general+appliances.service;
  const combinedGenerator=general+appliances.generator;
  ...
  const serviceFirst=Math.min(Math.max(demandServiceCombined,0),10000);
  const generatorFirst=Math.min(Math.max(demandGeneratorCombined,0),10000);
  const serviceRemainder=Math.max(demandServiceCombined-10000,0)*.40;
  const generatorRemainder=Math.max(demandGeneratorCombined-10000,0)*.40;
  ...
  if(typeof APPLIANCES!=='undefined'){
    APPLIANCES.forEach(function(item){
      body+=loadRow(applianceLabel(item),qty('q'+item.row),out('e'+item.row),out('f'+item.row));
    });
  }
  ...
  const hvacRows=[37,38,39,40].map(function(row){return loadRow(hvacLabel(row),qty('q'+row),out('e'+row),out('f'+row));}).join('');
  if(hvacRows){body+=sectionRow('HVAC Load')+hvacRows;}

  let continuousRows='';
  continuousRows+=loadRow('EV Charger',qty('q43'),out('e43'),out('f43'));
  const desc100=value('d47')||'Additional Continuous Load (100%)';
  continuousRows+=loadRow(desc100,qty('q47'),out('e47'),out('f47'));
  const desc125=value('d42')||'Additional Continuous Load (125%)';
  continuousRows+=loadRow(desc125,qty('q42'),out('e42'),out('f42'));
  if(data.largestMotor&&Number(data.largestMotor.additionalVA)>0){
    continuousRows+=loadRow((data.largestMotor.type||'Largest Motor')+' — Additional 25%','',data.largestMotor.additionalVA,data.largestMotor.additionalVA);
  }
...
<div class="print-page">\
  <div class="generator-print-heading">\
    <div class="print-brand"><span class="print-brand-main">LoadCalc</span><span class="print-brand-accent">Pro X</span></div>\
    <div class="print-title-text">Generator Optional Method Calculator</div>\
  </div>\
  <div class="print-project">\
    <div><strong>Project:</strong> '+esc(value('projectName'))+'</div>\
    <div><strong>Project #:</strong> '+esc(value('projectNumber'))+'</div>\
